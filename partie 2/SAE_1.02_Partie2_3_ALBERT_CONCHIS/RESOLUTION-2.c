 ...
 *
 * @page Général
 * @brief Programme de completion de Sudoku
 * @author ALBERT Cyprien - CONCHIS Valentin
 * @version 1.0
 * @date 22 Janvier 2023
 * 
 * Le programme si dessous a été amélioré en y ajoutant :
 *      - les méthodes de Singleton nu et Singleton caché
 *      - les méthodes de Paires nues et Paires cachées (partiellement correct)
 *      - un nouveau type de case nommé tCase2
 *      - d'autres fonctions et procédures permettant de traiter les méthodes
 * 
 * Le programme principal utilise toutes les méthodes jusqu'au moment où la grille
 * n'est plu modifié (valeurs ou candidats). Ensuite il finit de remplir la grille
 * (si possible) avec le backtracking
 * 
*/
...
void ajouterCandidat(tCase2*,int);                              // procédure ajoutée
void retirerCandidat(tCase2*,int);                              // procédure ajoutée
void retirerCandidatLigneColonneBloc(tGrille,int,int,int);      // procédure ajoutée
void retirerCandidatBlocPairesNues(tGrille,int,int,int,int);    // procédure ajoutée
bool estCandidat(tCase2,int);                                   // fonction ajoutée
void viderCandidatCase(tCase2*);                                // procédure ajoutée
bool estSurLigneColonneBloc(tGrille,int,int,int);               // fonction améliorée
void remplirCandidat(tGrille);                                  // procédure ajoutée
void afficheLigneSeparateur();
void afficherGrille(tGrille);
bool singletonNu(tGrille);                                      // fonction ajoutée
bool verifSingletonCache(tGrille,int,int,int);                  // fonction ajoutée
void singletonCache(tGrille,bool*);                             // procédure ajoutée
bool sontTableauxIdentiques(tCase2,tCase2);                     // fonction ajoutée
bool paireNuesIndividuel(tGrille, int,int,int,int);             // fonction ajoutée
void pairesNues(tGrille,bool*);                                 // procédure ajoutée
void pairesCache(tGrille,bool*);                                // procédure ajoutée
...
int main(){
    ...
    if(bt == true){
        ...
        printf("  La grille ne peut pas être remplie\n\n");
    }
    return EXIT_SUCCESS;
}

// Place le contenu d'un fichier binaire dans une grille de Sudoku
// en plaçant la valeur de la case dans la structure tCase2
...
// Amélioration : Traite les lignes, colonnes et bloc dans une même boucle au lieu d'être traité séparément
...
// La fonction est la méthode de paires nues appliquée à toute la grille
...
// La fonction est la méthode de paires cachées appliquée à toute la grille
...
// test tous les candidats de chaque case dans l'ordre, jusqu'à que la grille soit totalement remplie
// retourne false si la grille est remplie et false sinon
